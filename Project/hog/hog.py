...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    raw_score = player_score + take_turn(num_rolls, player_score, opponent_score, dice)
    return sus_points(raw_score)
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    while i <= 10:
        next = make_averaged(roll_dice, times_called)(i, dice)
        if next > max:
            index = i
            max = next
        i = i + 1
