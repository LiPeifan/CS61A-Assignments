def divide(quotients, divisors):
    ...
    """
    return {n: [m for m in divisors if m%n==0] for n in quotients}
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    ax, ay = get_lat(city_a), get_lon(city_a)
    bx, by = get_lat(city_b), get_lon(city_b)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis_a = distance(city_a, temp)
    dis_b = distance(city_b, temp)
